...
router.patch('/:id', (req, res) => {
    Meal.findOne({id: req.params.mealId}, (error, meal) => {
        if(error || !meal) {
...
router.delete('/:id', (req, res) => {
    Meal.findOne({id: req.params.id}, (error, meal) => {
        if(error || !meal) {
            res.status(500).json({
        ...
        .catch(error => {
            res.status(500).json({error: 'Unable to delete meal ' + req.params.id});
        });
    });
});
